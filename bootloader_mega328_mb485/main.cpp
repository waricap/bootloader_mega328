...
{ 	
	...
	// _PROTECTED_WRITE(CPUINT.CTRLA, (CPUINT.CTRLA | CPUINT_IVSEL_bm));
	 //Enable global interrupts
		...
		DDRC  = 0xFF; // везде назначаем вЫхода,
...
//		sei();
		...
		ucMBAddress = 2 ;  // ==== ДЛЯ  ПЕРЕПРОШИВКИ   !!!!!!!!!!!!!!!такой адрес будет у всех плат УЗ-ванна
		...
		wdt_enable(WDTO_4S);// включаем сторожевой таймер
    ...
    while (1) 
    {
		...
		if ( MB_ENOERR == eMBPoll() )
			{			
				...
				// usReg_HOLDING_Buf[2]=0;		// номер  страница, которую надо записать во флеш, этот регистр читает мастер, и поэтому знает и потом пришлет что надо, но потом.
				usReg_HOLDING_Buf[3] = 0x7000;	// допустимый для записи размер памяти
				usReg_HOLDING_Buf[4] = 1;		// версия
				//for (i=5; i<72; i++) 
				//{
				...
				if ((usReg_HOLDING_Buf[69] > 0) & (usReg_HOLDING_Buf[69] < (0x7000 / SPM_PAGESIZE +2)))// только если с мастера придет какой-то осмысленый пакет с данными, только тогда пытаться прошивать новые данные
				{
					page = (usReg_HOLDING_Buf[69]-1);	// usReg_HOLDING_Buf[69] - тут приходит НОМЕР страницы (+1), который записан в конце пришедшего пакета данных
					// page - это НОМЕР страницы флэша, которую надо записать-стереть-прочитать
					// ============================ write boot ========================== //
					if (page == usReg_HOLDING_Buf[2])
					{
						adr8_page = page * SPM_PAGESIZE; // adr8_page - это НЕ номер страницы флэша, это есть  8-битовый адрес начала страницы, которую надо записать-стереть-прочитать
						cli();		// отключаем прерывания
							boot_page_erase(adr8_page);		// стереть память
							boot_spm_busy_wait ();      // wait, пока память не будет стерта.
							for (i=0; i<(SPM_PAGESIZE/2); i++) // SPM_PAGESIZE  128 байт  == 64 слова
							{
								boot_page_fill (adr8_page + i*2, usReg_HOLDING_Buf[i+5]);  // заполняем буфер для записи,  page - номер страницы во flash
							}
							boot_page_write (adr8_page); // записать данные из буфера на флэш-страницу.  page - номер страницы во flash
